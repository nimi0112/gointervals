import { FIELDS, draftFrom, resolveDraft, stepField } from '@/islands/fields';
...
describe('fields', () => {
  ...
  it('draftFrom shows meditation in minutes and everything else as stored', () => {
    ...
    expect(draftFrom(defaultConfigs.interval)).toEqual({ work: '1800', rest: '300', rounds: '8' });
  ...
  it('resolveDraft reports every invalid field with its message', () => {
    ...
    expect(r).toEqual({
      ok: false,
      errors: { work: 'Work must be at least 1 second.', rest: 'Enter a number.' },
    });
  });
